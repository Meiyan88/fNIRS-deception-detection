...
class fNIRS_T(nn.Module):
    ...
    def __init__(self, n_class, sampling_point, dim, depth, heads, mlp_dim, pool='cls', dim_head=64, dropout=0., emb_dropout=0.):
        ...
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(135),
            nn.Linear(135, 39))
        self.fc1 = nn.Linear(39, 2)




    def forward(self, img,sheet, mask=None):
        x = self.to_patch_embedding(img)
        x2 = self.to_channel_embedding(img) # .squeeze()

        b, n, _ = x.shape
        cls_tokens = repeat(self.cls_token_patch, '() n d -> b n d', b=b)  #input, pattern(yingsheguanxi),按照指定的规格形式进行变换)
